...
## Separate the review job from verification

The scheduled Luna job has source, Git history and a bounded editing window.
It does not have a built Joolia. It must not run builds, install dependencies or
claim recommended checks passed. It can add focused tests and inspect code.
Build/test jobs receive the resulting source tree without the OpenAI credential.

A report's decision=propose means ready for a draft and CI, not ready to merge.
...
python3 contrib/ci/run.py --stage pkg --stage repl --logs /tmp/joolia-sync-ci
```
...
5. If diagnosis exceeds the allowed pass/budget, report the blocker and stop.
   This initial automation has no unattended build-repair or bisection loop.
