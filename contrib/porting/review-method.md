...
Use this guide, subsystems.md, validation.md and contract.md for deliberate
upstream integrations. This branch is pinned; there is no scheduled agent or
per-commit job assignment. Review an aggregate update by subsystem and inspect
individual commits wherever their intent or dependencies matter.

## Establish the source of truth

Record four revisions before an update:

- `previous_sha`: the exact upstream revision underlying the pinned port.
- `base_sha`: Joolia before this proposal, including all local adaptations.
- `target_sha`: the selected upstream release or reviewed milestone.
- `fetched_sha`: the upstream tip observed; this is context, not an update target.

A prepared merge is not a released or validated update. Inspect merge-only
resolutions as well as ordinary changes. Change the baseline record only after
review and successful verification of the complete candidate.
...
When no built Joolia is available, record build commands as required follow-up;
never claim they ran.

## Review changed subsystems in three views

For each changed subsystem and relevant commit, inspect:
...
outside it, identify that SHA and leave the proposal unresolved. Do not cherry-pick unrelated
...
## Write an auditable review record

For each subsystem, record relevant commit SHAs and:
...
Leave the proposal unmerged and explain the exact boundary when:

- The integration has merge conflicts, infrastructure changes or required
