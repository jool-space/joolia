...
function precompile(
    ...
    return activate(dirname(ctx.env.project_file)) do
        ...
        return Base.Precompilation.precompilepkgs(pkgs_name; internal_call, strict, warn_loaded, timing, _from_loading, configs, manifest = workspace, io, detachable, skip_dependents, force)
    end
end
