    ...
    "package" => CommandDeclaration[
        ...
        PSA[
            ...
            :help => md"""
                ...
                Dependents of packages that fail to precompile are skipped. Pass `--noskip` to attempt
                them anyway, for example when a package only loads that dependency on some platforms.
                Pass `--force` to recompile packages with fresh caches; standard libraries are left alone.
