...
function _Meta_parse_string(text::AbstractString, filename::AbstractString,
    ...
    ex, offset::Int = JuliaSyntax.core_parser_hook(text, filename, lineno, index, options)
    ex, offset
end
...
function Meta_parse(str::AbstractString;
    ...
    ex, pos = Meta_parse(str, 0; filename=filename, greedy=true, raise=raise, depwarn=depwarn)
...
function Meta_parseall(text::AbstractString; filename="none", lineno=1)
    ex,_ = _Meta_parse_string(text, String(filename), lineno, 0, :all)
    return ex
end
