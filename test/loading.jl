    ...
    begin
        ...
        if !Sys.iswindows()
            ...
            if length(precompiling) == 2
                ...
                @test endswith(strip(precompiling[0]), "Precompiling ExtDep2")
                @test !occursin("]", precompiling[1])
                for name in ("HasExtensions", "ExtDep", "HasExtensions → Extension",
                             "HasExtensions → ExtensionDep", "HasExtensions → ExtensionFolder")
                    @test occursin(name, precompiling[1])
                end
            end
            ...
            @test count("successfully precompiled", output) == 2
            if length(precompiling) == 2
                @test occursin("Precompiling HasExtensions", precompiling[0])
                @test !occursin("]", precompiling[1])
