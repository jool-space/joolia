...
function recognize_global_read(@nospecialize(stmt), ir::IRCode)
    ...
    f = _global_call_singleton(stmt.args[0], ir)
    if f === Core.getglobal
        M = _global_call_singleton(stmt.args[1], ir)
        s = _global_call_singleton(stmt.args[2], ir)
...
function _global_write_target(stmt::Expr, base::Int, ir::IRCode)
    M = _global_call_singleton(stmt.args[base+1-1], ir)
    s = _global_call_singleton(stmt.args[base+2-1], ir)
...
function recognize_global_write(@nospecialize(stmt), ir::IRCode)
    ...
    f = _global_call_singleton(stmt.args[base-1], ir)
    if f === Core.modifyglobal! && (na == 4 || na == 5)
        g = _global_write_target(stmt, base, ir)
        g === nothing && return nothing
        order = optional_order_arg(stmt, base+5, ir)
        return GlobalWriteInfo(g, :modifyglobal_partition, order, nothing, stmt.args[base+4-1], stmt.args[base+3-1], invoke)
    ...
    elseif f === Core.setglobal! && (na == 3 || na == 4)
        ...
        return GlobalWriteInfo(g, :setglobal_partition, order, nothing, stmt.args[base+3-1], nothing, nothing)
    elseif f === Core.swapglobal! && (na == 3 || na == 4)
        g = _global_write_target(stmt, base, ir)
        g === nothing && return nothing
        order = optional_order_arg(stmt, base+4, ir)
        return GlobalWriteInfo(g, :swapglobal_partition, order, nothing, stmt.args[base+3-1], nothing, nothing)
    elseif f === Core.replaceglobal! && (4 <= na <= 6)
        g = _global_write_target(stmt, base, ir)
        g === nothing && return nothing
        order = optional_order_arg(stmt, base+5, ir)
        failorder = optional_order_arg(stmt, base+6, ir)
        return GlobalWriteInfo(g, :replaceglobal_partition, order, failorder, stmt.args[base+4-1], stmt.args[base+3-1], nothing)
    elseif f === Core.setglobalonce! && (3 <= na <= 5)
        g = _global_write_target(stmt, base, ir)
        g === nothing && return nothing
        order = optional_order_arg(stmt, base+4, ir)
        failorder = optional_order_arg(stmt, base+5, ir)
        return GlobalWriteInfo(g, :setglobalonce_partition, order, failorder, stmt.args[base+3-1], nothing, nothing)
    end
    return nothing
end
...
function recognize_global_isdefined(@nospecialize(stmt), ir::IRCode)
    ...
    if f === Core.isdefinedglobal && (3 <= na <= 5)
        ...
        if na >= 4
            _global_call_singleton(stmt.args[3], ir) === true || return nothing
        end
...
function _reformulate_globals!(ir::IRCode, opt::OptimizationState)
    ...
    for idx = 1:length(ir.stmts)
        ...
        if d !== nothing
            ...
            if rr !== nothing
                stmt = Expr(:call, GlobalRef(Core, :isdefinedglobal_partition), QuoteNode(rr[0]), d.second)
                inst[:stmt] = stmt
